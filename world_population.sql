...
## Identify top countries by population and their economic ranking.
...
WITH country_metrics AS (
    SELECT
        ...
        CASE 
            WHEN Population > 0 THEN GNP / Population 
            ELSE NULL 
...
ranked AS (
    SELECT
        ...
        RANK() OVER (ORDER BY GNP DESC) AS GDP_Rank,
        RANK() OVER (ORDER BY GDP_Per_Capita DESC) AS GDP_Per_Capita_Rank
...
## Determine the most spoken official languages based on population coverage.


SELECT 
    cl.Language,
    SUM(c.Population) AS TotalPopulationCoverage,
...
GROUP BY 
    cl.Language
ORDER BY 
    TotalPopulationCoverage DESC;


## Analyze population distribution across continents.

SELECT sum(Population) AS Total_Population, Continent FROM country group by continent ORDER by Total_Population DESC;

## Find the largest and smallest cities by population in each continent.

WITH ranked_cities AS (
    SELECT 
        cty.Name AS city_name,
        cty.Population AS city_population,
        cntry.Name AS country_name,
        cntry.Continent,
        RANK() OVER (PARTITION BY cntry.Continent ORDER BY cty.Population DESC) AS rank_largest,
        RANK() OVER (PARTITION BY cntry.Continent ORDER BY cty.Population ASC) AS rank_smallest
    FROM 
        city cty
    JOIN 
        country cntry ON cty.CountryCode = cntry.Code
)
SELECT 
    Continent,
    MAX(CASE WHEN rank_largest = 1 THEN city_name END) AS largest_city,
    MAX(CASE WHEN rank_largest = 1 THEN city_population END) AS largest_city_population,
    MAX(CASE WHEN rank_largest = 1 THEN country_name END) AS largest_city_country,
    MAX(CASE WHEN rank_smallest = 1 THEN city_name END) AS smallest_city,
    MAX(CASE WHEN rank_smallest = 1 THEN city_population END) AS smallest_city_population,
    MAX(CASE WHEN rank_smallest = 1 THEN country_name END) AS smallest_city_country
FROM 
    ranked_cities
WHERE 
    rank_largest = 1 OR rank_smallest = 1
GROUP BY 
    Continent
ORDER BY 
    Continent;
    
## Identify countries with the highest and lowest GDP per capita.

-- Top 5 highest GDP per capita countries
SELECT 
    Name AS country,
...
FROM 
    country
WHERE 
    GNP > 0 AND Population > 0
ORDER BY 
    gdp_per_capita DESC
LIMIT 5;

-- Top 5 lowest GDP per capita countries
SELECT 
    Name AS country,
    (GNP / Population) AS gdp_per_capita,
    'Lowest' AS category
...
## Determine language diversity per continent.

SELECT 
    c.Continent,
    COUNT(DISTINCT cl.Language) AS number_of_languages,
    ROUND(COUNT(DISTINCT cl.Language) / COUNT(DISTINCT c.Code) * 100, 2) AS languages_per_country
FROM 
    country c
JOIN 
...
## Identify the correlation between population size and life expectancy.
